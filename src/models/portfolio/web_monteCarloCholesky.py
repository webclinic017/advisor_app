...
class MonteCarloCholesky(object):
    ...
    def import_stock_data(self, tickers, start='2010-1-1', end=datetime.today().strftime('%Y-%m-%d')):
        data = pd.DataFrame(yf.download(tickers, start=start)['Adj Close'])
        return data
    ...
    def montecarlo_cholesky(self, tickers, days, iterations, start='2010-1-1', show_hist=False, show_line=False, show_stats=True):
      # Get tickers and number of tickers involved in this portfolio
        data = self.import_stock_data(tickers, start=start)
      ...
      # For every stock, calculate the returns
        for s in range(len(ticks)):
          ...
          #Printing basic stats
                ...
                y=finvizfinance(ticks[s]).TickerFundament()["Target Price"]
    ...
    def beta_sharpe(self, tickers, data, mark_ticker="^GSPC", start='2010-1-1', riskfree=0.025, riskpremium=0.05):
      # Beta
        dd, mark_ret = self.market_data_combination(tickers, data, mark_ticker, start)   # load data and annual returns
    ...
    def montecarlo_sharpe_optimal_portfolio(self, tickers, trials, riskfree=0.025, plot_eff = True, start_date = '2010-1-1', end_date=None):
        if end_date == None:
...
if __name__ == '__main__':
    t_lst = ['GOOG','FB','^GSPC']
    MonteCarloCholesky().montecarlo_cholesky(t_lst, days=252, iterations=10000, start='2015-1-1', show_hist=True)

    ticker_lst = ['AAPL','ASML','NVDA','SNOW','PLTR','TSLA']
    no_trials = 1000
    fin = MonteCarloCholesky().montecarlo_sharpe_optimal_portfolio(ticker_lst, no_trials)
    st.dataframe(fin)