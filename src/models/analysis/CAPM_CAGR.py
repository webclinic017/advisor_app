...
class CAPM_CAGR(object):
    ...
    def import_stock_data(self, tickers, start='2010-1-1', end=datetime.today().strftime('%Y-%m-%d')):
        data = pd.DataFrame(yf.download(tickers, period='10y')['Adj Close'])
        return data
    ...
    def stock_CAPM(self, stock_ticker, market_ticker, start_date = '2010-1-1', riskfree = 0.025, riskpremium = 'set'):
...
if __name__ == '__main__':
    CAPM_CAGR().configure_mod(ticker_lst=['AAPL','AMZN','MSFT','GOOGL','NFLX','DIS'])