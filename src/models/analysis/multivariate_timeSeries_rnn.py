...
saveFigger = Path(f"data/variates/multivariate/{saveMonth}/{saveDate}/")
if not saveFigger.exists():
    saveFigger.mkdir(parents=True)
...
class Multivariate_TimeSeries_RNN(object):
    ...
    def multivariate(self):
        try:
            ...
            lstm_path = ("data/variates/lstm.h5").as_posix()
            ...
            axes[0].fill_between(
                x=y_test.index, y1=0.0, y2=1.4, color="grey", alpha=0.3
            )
            ...
            # st.pyplot(fig)
            plt.savefig
            fig.savefig(saveFigger / f"multivariateTS.png", dpi=100)
