...
class Regression_Model(object):
    ...
    def linear_regression(self, dfreg, X_lately, clfreg, clfpoly2, clfpoly3, clfknn, days=126):
        ...
        plt.title(
            f"{self.sName} ({self.ticker}) - Linear Regression - 6 Month Forecast",
            fontsize=30,
            fontweight="bold",
        )
        ...
        st.subheader(
            f" > {self.sName} · [{self.ticker}] · Current Stock Price = **${x}**"
        )
        x = dfreg["Forecast"].iloc[-1]
        st.subheader(f" > 6 Month Forecasted Price = **${round(float(x),2)}**")
        return

    def quadratic_regression_2(self, dfreg, X_lately, clfreg, clfpoly2, clfpoly3, clfknn, days=126):
        ...
        dfreg["Forecast"].tail(500).plot()
        plt.title(
            f"{self.sName} ({self.ticker}) - Quadratic (2) Regression - 6 Month Forecast",
            fontsize=30,
            fontweight="bold",
        )
        plt.xlabel("Date", fontsize=20, fontweight="bold")
    ...
    def quadratic_regression_3(self, dfreg, X_lately, clfreg, clfpoly2, clfpoly3, clfknn, days=126):
        ...
        dfreg["Forecast"].tail(500).plot()
        plt.title(
            f"{self.sName} ({self.ticker}) - Quadratic (3) Regression - 6 Month Forecast",
            fontsize=30,
            fontweight="bold",
        )
        plt.xlabel("Date", fontsize=20, fontweight="bold")
        ...
        x = round(float(fd["Adj Close"]), 2)
        st.subheader(
            f" > {self.sName} · [{self.ticker}] · Current ticker Price = **${x}**"
        )
        x = dfreg["Forecast"].iloc[-1]
        st.subheader(f" > 6 Month Forecasted Price = **${round(float(x),2)}**")
        return

    def knn(self, dfreg, X_lately, clfreg, clfpoly2, clfpoly3, clfknn, days=126):
        ...
        dfreg["Forecast"].tail(500).plot()
        plt.title(
            f"{self.sName} ({self.ticker}) - KNN Regression - 6 Month Forecast",
            fontsize=30,
            fontweight="bold",
        )
        plt.xlabel("Date", fontsize=20, fontweight="bold")
...
if __name__ == "__main__":
    ...
    if res_lst[-1] == "linear_regression":
        Regression_Model(stock_ticker).linear_regression(
            res_lst[0], res_lst[1], res_lst[2], res_lst[3], res_lst[4], res_lst[5], no_days
            )
